...
For smooth 60 fps rendering of the 30fps simulation there is a smoothing factor applied to interpolate `currentPos` towards `targetPos` for each instance. To avoid the initial swoosh from the creation position (out of sight at -100 depth),
